...
\end{tikzpicture}
\caption{Latency overhead of a GPU and CPU computational model}
...
\def\xend{9}   % 8 intervals = 9 points
...
\node[above] at (\xend - 0.0, \tableHeight + 0.6) {$y_8$};
...
% Consider including the python code showing the actual operations of the original CNN so they get a sense of what a forward pass entails
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Function{Forward}{$\text{model}, \text{input}$}
            ...
            \For{$i = 1$ \textbf{to} $\text{model}.\text{len} - 1$}
